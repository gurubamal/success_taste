...
if [ "$HOSTNAME" = node7 ]; then
	...
	sudo kubeadm init --apiserver-advertise-address=192.168.58.7 --pod-network-cidr=192.168.0.0/16 |tee init.txt ; echo sudo $(tail -2 init.txt|head -1| cut -d'\' -f1)  $(tail -1 init.txt| cut -d'[' -f1) |tee -a  compute_add.sh
	JOIN_CMD=$(cat compute_add.sh)
	echo   "if ! [ $HOSTNAME = node7 ]; then
	...
	sed 's/node7 = node7/\$HOSTNAME = node7/g' compute_add.sh |tee /vagrant/compute_add.sh	
	chmod +x /vagrant/*.sh
	else exit 0
	fi
fi
