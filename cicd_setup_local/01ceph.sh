...
if ! grep -q "192.168.58" /etc/hosts; then
  {
    ...
    echo "192.168.58.9        node9  controller"
...
if ! grep -q "PermitRootLogin yes" /etc/ssh/sshd_config; then
  ...
  if [ $? -ne 0 ]; then
    echo "Error: Failed to update PermitRootLogin in sshd_config."
  fi
fi

# Ensure StrictHostKeyChecking is set to no
if ! grep -q "StrictHostKeyChecking no" /etc/ssh/ssh_config; then
  ...
  if [ $? -ne 0 ]; then
    echo "Error: Failed to update StrictHostKeyChecking in ssh_config."
  fi
fi
