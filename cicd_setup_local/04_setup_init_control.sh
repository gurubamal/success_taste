...
if [ "$HOSTNAME" = node6 ]; then
	...
	sudo kubeadm init --apiserver-advertise-address=192.168.58.6 --pod-network-cidr=192.168.0.0/16 |tee init.txt ; echo sudo $(tail -2 init.txt|head -1| cut -d'\' -f1)  $(tail -1 init.txt| cut -d'[' -f1) |tee -a  compute_add.sh
